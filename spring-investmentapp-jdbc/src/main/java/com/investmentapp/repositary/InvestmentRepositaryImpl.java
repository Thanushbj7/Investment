...
@Repository
public class InvestmentRepositaryImpl implements IInvestmentRepositary {
...
@Autowired
	...
	public void updateInvestment(int planId, double amount) {
		...
		jdbcTemplate.update(sql,planId,amount);
	}

	@Override
	public void deleteInvestment(int planIdt) {
		
		jdbcTemplate.update(DBQueries.DELETEQUERY,planIdt);
	}
	...
	public List<Investment> findByPurpose(String purpose) {
		List<Investment> investment=jdbcTemplate.query(DBQueries.SELECTBYPURPOSE,new RowMapper<Investment>() {
	...
	public List<Investment> findByRiskAndTerm(String risk, int term) {
		...
		List<Investment> investment=
				jdbcTemplate.query(DBQueries.SELECTBYRISKTERM,mapper,risk,term);
		return investment;
	...
	public Investment findById(int planId) {
		Investment investments=jdbcTemplate.queryForObject(DBQueries.SELECTBYID,
				(rs,rowNum)->{
				Investment investment=new Investment();
				investment.setPlanName(rs.getString("plan_name"));
				investment.setPurpose(rs.getString("purpose"));
