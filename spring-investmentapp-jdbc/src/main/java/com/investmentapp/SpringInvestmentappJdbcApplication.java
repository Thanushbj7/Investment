package com.investmentapp;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.CommandLineRunner;
import org.springframework.boot.SpringApplication;
import org.springframework.boot.autoconfigure.SpringBootApplication;

import com.investmentapp.model.Investment;
import com.investmentapp.model.Nominee;
import com.investmentapp.model.PlanType;
import com.investmentapp.model.Purpose;
import com.investmentapp.model.Risk;
import com.investmentapp.service.IInvestmentService;

@SpringBootApplication
public class SpringInvestmentappJdbcApplication implements CommandLineRunner{

	public static void main(String[] args) {
		SpringApplication.run(SpringInvestmentappJdbcApplication.class, args);
	}
	@Autowired
	IInvestmentService investmentService;
	@Override
	public void run(String... args) throws Exception {
//		Investment investment= new Investment("Voya Mutual",10,PlanType.mutual.type,9000,Purpose.MARRAIGE.name(),Risk.HIGH.name(),Nominee.SON.name(),10);
//		Investment investment1= new Investment("Voya Mutual",11,PlanType.fd.type,8000,Purpose.RETIREMENT.name(),Risk.HIGH.name(),Nominee.SON.name(),11);
//		investmentService.addInvestment(investment1);
		Investment investment=new Investment();
//		investmentService.getAll().forEach(System.out::println);
//		investmentService.getByPurpose("MARRAIGE").forEach(System.out::println);
//		investmentService.getByRiskAndTerm("HIGH", 10).forEach(System.out::println);
		System.out.println(investmentService.getById(1));
	}

}
